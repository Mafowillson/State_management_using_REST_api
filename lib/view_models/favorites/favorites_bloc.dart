import 'dart:convert';
import 'dart:developer';

import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:equatable/equatable.dart';
import 'package:mvvm_architecture_in_flutter/models/movies_model.dart';
import 'package:shared_preferences/shared_preferences.dart';

part 'favorites_event.dart';
part 'favorites_state.dart';

class FavoritesBloc extends Bloc<FavoritesEvent, FavoritesState> {
  FavoritesBloc() : super(FavoritesInitial()) {
    on<LoadFavorites>(_loadFavorites);
    on<AddToFavorites>(_addToFavorites);
    on<RemoveFromFavorites>(_removeFromFavorites);
    on<RemoveAllFromFavorites>(_clearAllFavs);
  }

  final favsKey = 'favKey';

  Future<void> _saveFavorites(List<MoviesModel> favoritesList) async {
    final prefs = await SharedPreferences.getInstance();

    final stringList =
        favoritesList.map((movies) => json.encode(movies.toJson())).toList();
    prefs.setStringList(favsKey, stringList);
  }

  // bool _isFavorites(MoviesModel moviesModel) {
  //   if (state is FavoritesLoaded) {
  //     return (state as FavoritesLoaded)
  //         .favorites
  //         .any((movie) => movie.id == moviesModel.id);
  //   }
  //   return false;
  // }

  Future<void> _addToFavorites(AddToFavorites event, emit) async {
    // emit(FavoritesError(message: 'An error has occured'));
    // throw Exception('An error has occured');
    if (state is FavoritesInitial) {
      log('state is favoriteinitial');
    }
    if (state is FavoritesLoaded) {
      log('state is favoriteLoaded');
      List<MoviesModel> updatedFavorites =
          List.from((state as FavoritesLoaded).favorites)
            ..add(event.moviesModel);
      emit(FavoritesLoaded(favorites: updatedFavorites));
      log('updatedFavorites length in add ${updatedFavorites.length}');
      await _saveFavorites(updatedFavorites);
    }
  }

  Future<void> _removeFromFavorites(RemoveFromFavorites event, emit) async {
    if (state is FavoritesLoaded) {
      List<MoviesModel> updatedFavorites = (state as FavoritesLoaded)
          .favorites
          .where((movie) => movie.id != event.moviesModel.id)
          .toList();
      emit(FavoritesLoaded(favorites: updatedFavorites));
      log('updatedFavorites length in remove ${updatedFavorites.length}');
      await _saveFavorites(updatedFavorites);
    }
  }

  Future<void> _loadFavorites(event, emit) async {
    final prefs = await SharedPreferences.getInstance();

    final stringList = prefs.getStringList(favsKey) ?? [];

    final favsList = stringList
        .map((movie) => MoviesModel.fromJson(json.decode(movie)))
        .toList();
    emit(FavoritesLoaded(favorites: favsList));
  }

  void _clearAllFavs(event, emit) {
    emit(FavoritesLoaded(favorites: []));
    _saveFavorites([]);
  }
}
